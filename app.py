...
def patshowdoctor():
    ...
    if request.method == "POST":
        ...
        print(key)
        docs = doctor_table.find(
            {'speciality': {'$regex': key, "$options": "i"}})
...
def favtips(id):
    ...
    email = session["email_pat"]
    # this is storing a users favourite tips
    fa = fav_tips.insert_one(
        {"title": ti["title"], "body": ti["body"], "email": email})
...
def favmedi(id):
    ...
    email = session["email_pat"]
    # this is storing a users favourite tips
    fa = fav_medicine.insert_one(
        {"name": ti["name"], "type": ti["disease_type"], "desc": ti["desc"], "email": email})
...
def editdoctor(id):
    ...
    if request.method == "POST":
        ...
        doctor_table.update_many({'_id': ObjectId(id)}, {"$set": {
                                 "email": email, "contact": contact, "name": name, "speciality": speciality, "institution": institution}})
        return redirect("/showdoctor")
...
def createmedicine():
    ...
    if request.method == 'POST':
        ...
        for upload in request.files.getlist("image"):  # multiple image handel
            ...
            medi_table.insert_one(
                {'name': name, 'disease_type': disease_type, 'desc': desc, 'image': filename})
...
def editmedicine(id):
    ...
    if request.method == 'POST':
        ...
        for upload in request.files.getlist("image"):  # multiple image handel
            ...
            destination = "/".join([target, filename])
            upload.save(destination)
            medi_table.update_many({'_id': ObjectId(id)}, {"$set": {
                                   'name': name, 'disease_type': disease_type, 'desc': desc, 'image': filename}})
...
def edittips(id):
    ...
    if request.method == "POST":
        ...
        tips_table.update_many({'_id': ObjectId(id)}, {
                               "$set": {"title": form_title, "body": form_body}})
        return redirect("/showtips")
